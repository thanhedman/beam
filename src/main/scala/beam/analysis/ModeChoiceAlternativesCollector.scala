...
class ModeChoiceAlternativesCollector(beamServices: BeamServices) extends BasicEventHandler with LazyLogging {
  ...
  def iterationEnds(event: IterationEndsEvent): Unit = {
    csvWriter.close()
    logger.info("CsvWriter closed for file " + csvFilePath)
  }

  def iterationStarts(event: IterationStartsEvent): Unit = {

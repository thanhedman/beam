  ...
  "Running Async Alonso Mora Algorithm" must "creates a consistent plan" in {
    val config = ConfigFactory
      .parseString("""
                     ...
                     |beam.agentsim.agents.rideHail.allocationManager.alonsoMora.maxSizeOfSolutionSpacePerVehicle = 1000
        """.stripMargin)
  ...
  "Running Async Alonso Mora Algorithm" must "scale" in {
    val config = ConfigFactory
      .parseString("""
                     ...
                     |beam.agentsim.agents.rideHail.allocationManager.alonsoMora.maxSizeOfSolutionSpacePerVehicle = 100
        """.stripMargin)
